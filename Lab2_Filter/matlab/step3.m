...
subplot(2,2,1);
semilogy(b_scan, rmse_in, '-o', 'LineWidth', 1.5);
yline(rmse_threshold, 'r--', 'Threshold (2^{-11})');
title('(a) RMSE vs Input Word-length');
xlabel('Word-length (bits)'); ylabel('RMSE (log scale)'); grid on;

subplot(2,2,2);
semilogy(b_scan, rmse_coef, '-o', 'LineWidth', 1.5);
yline(rmse_threshold, 'r--', 'Threshold');
title('(b) RMSE vs Coef Word-length');
xlabel('Word-length (bits)'); ylabel('RMSE (log scale)'); grid on;

subplot(2,2,3);
semilogy(b_scan, rmse_mult, '-o', 'LineWidth', 1.5);
yline(rmse_threshold, 'r--', 'Threshold');
title('(c) RMSE vs Multiplier Output Word-length');
xlabel('Word-length (bits)'); ylabel('RMSE (log scale)'); grid on;

subplot(2,2,4);
semilogy(b_scan, rmse_add, '-o', 'LineWidth', 1.5);
yline(rmse_threshold, 'r--', 'Threshold');
title('(d) RMSE vs Adder Output Word-length');
xlabel('Word-length (bits)'); ylabel('RMSE (log scale)'); grid on;
exportgraphics(gcf, fullfile(fig_dir, 'step3_wordlength_decision.png'), 'Resolution', 150);
...
exportgraphics(gcf, fullfile(fig_dir, 'step3_time_domain_comparison.png'), 'Resolution', 150);
...
exportgraphics(gcf, fullfile(fig_dir, 'step3_freq_domain_comparison.png'), 'Resolution', 150);
