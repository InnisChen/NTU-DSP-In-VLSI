...
module tb_fft32_step9;
    ...
    reg signed [DATA_W-1:0] input_re [0:95];
    reg signed [DATA_W-1:0] input_im [0:95];
    reg signed [DATA_W-1:0] golden_sdf_re [0:95];
    reg signed [DATA_W-1:0] golden_sdf_im [0:95];
    reg signed [DATA_W-1:0] golden_br_re [0:95];
    reg signed [DATA_W-1:0] golden_br_im [0:95];
...
`ifdef SDF_SIM
    initial begin
        $sdf_annotate("fft32_top.sdf", dut, , , "MAXIMUM");
    end
`endif
    ...
    initial begin
        $readmemh({`FFT32_DAT_DIR, "/stream96_re.dat"}, input_re);
        $readmemh({`FFT32_DAT_DIR, "/stream96_im.dat"}, input_im);
        $readmemh({`FFT32_DAT_DIR, "/golden_sdf96_re.dat"}, golden_sdf_re);
        $readmemh({`FFT32_DAT_DIR, "/golden_sdf96_im.dat"}, golden_sdf_im);
        $readmemh({`FFT32_DAT_DIR, "/golden_br96_re.dat"}, golden_br_re);
        $readmemh({`FFT32_DAT_DIR, "/golden_br96_im.dat"}, golden_br_im);
    end
    ...
    initial begin
        ...
        err_count = 0;
        file_error = 0;
        fd_rtl_sdf_re = $fopen({`FFT32_DAT_DIR, "/rtl_sdf96_re.dat"}, "w");
        fd_rtl_sdf_im = $fopen({`FFT32_DAT_DIR, "/rtl_sdf96_im.dat"}, "w");
        fd_rtl_br_re = $fopen({`FFT32_DAT_DIR, "/rtl_br96_re.dat"}, "w");
        fd_rtl_br_im = $fopen({`FFT32_DAT_DIR, "/rtl_br96_im.dat"}, "w");
        ...
        $dumpfile("tb_fft32_step9.vcd");
        $dumpvars(0, tb_fft32_step9);
        ...
        if (err_count == 0 && file_error == 0 && sdf_count == 96 && br_count == 96) begin
            $display("============================================================");
            $display("[PASS] tb_fft32_step9");
            ...
            $display("============================================================");
            $display("[FAIL] tb_fft32_step9");
            $display("       SDFOut checked: %0d / 96 samples", sdf_count);
            $display("       BROut checked : %0d / 96 samples", br_count);
            $display("       Mismatches    : %0d", err_count);
    ...
    always @(posedge clk) begin
        if (sdf_valid_out && sdf_count < 96) begin
        ...
        if (br_valid_out && br_count < 96) begin
