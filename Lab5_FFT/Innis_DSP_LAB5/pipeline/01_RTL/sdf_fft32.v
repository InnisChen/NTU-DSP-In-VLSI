...
module sdf_fft32 #(
    ...
    reg signed [DATA_W-1:0] pipe1_re;
    reg signed [DATA_W-1:0] pipe1_im;
    reg signed [DATA_W-1:0] pipe2_re;
    reg signed [DATA_W-1:0] pipe2_im;
    reg signed [DATA_W-1:0] pipe3_re;
    reg signed [DATA_W-1:0] pipe3_im;
    reg signed [DATA_W-1:0] pipe4_re;
    reg signed [DATA_W-1:0] pipe4_im;
    reg signed [DATA_W-1:0] pipe5_re;
    reg signed [DATA_W-1:0] pipe5_im;
    ...
    sdf_stage #(
        ...
        .clk(clk),
        .rst_n(rst_n),
        .valid_in(pipe1_valid),
        .stage_in_re(pipe1_re),
        .stage_in_im(pipe1_im),
    ...
    sdf_stage #(
        ...
        .clk(clk),
        .rst_n(rst_n),
        .valid_in(pipe2_valid),
        .stage_in_re(pipe2_re),
        .stage_in_im(pipe2_im),
    ...
    sdf_stage #(
        ...
        .clk(clk),
        .rst_n(rst_n),
        .valid_in(pipe3_valid),
        .stage_in_re(pipe3_re),
        .stage_in_im(pipe3_im),
    ...
    sdf_stage #(
        ...
        .clk(clk),
        .rst_n(rst_n),
        .valid_in(pipe4_valid),
        .stage_in_re(pipe4_re),
        .stage_in_im(pipe4_im),
        .valid_out(stage5_valid),
        .stage_out_re(stage5_re),
        .stage_out_im(stage5_im)
    );

    assign sdf_valid_out = pipe5_valid;
    assign SDFOutRe = pipe5_re;
    assign SDFOutIm = pipe5_im;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            pipe4_valid <= 1'b0;
            pipe5_valid <= 1'b0;
            pipe1_re <= {DATA_W{1'b0}};
            pipe1_im <= {DATA_W{1'b0}};
            pipe2_re <= {DATA_W{1'b0}};
            pipe2_im <= {DATA_W{1'b0}};
            pipe3_re <= {DATA_W{1'b0}};
            pipe3_im <= {DATA_W{1'b0}};
            pipe4_re <= {DATA_W{1'b0}};
            pipe4_im <= {DATA_W{1'b0}};
            pipe5_re <= {DATA_W{1'b0}};
            pipe5_im <= {DATA_W{1'b0}};
        end else begin
            pipe1_valid <= stage1_valid;
            pipe1_re <= stage1_re;
            pipe1_im <= stage1_im;
            pipe2_valid <= stage2_valid;
            pipe2_re <= stage2_re;
            pipe2_im <= stage2_im;
            pipe3_valid <= stage3_valid;
            pipe3_re <= stage3_re;
            pipe3_im <= stage3_im;
            pipe4_valid <= stage4_valid;
            pipe4_re <= stage4_re;
            pipe4_im <= stage4_im;
            pipe5_valid <= stage5_valid;
            pipe5_re <= stage5_re;
            pipe5_im <= stage5_im;
        end
    end
...
module sdf_stage #(
    ...
    generate
        if (TWIDDLE_MODE == 0) begin : gen_twiddle_full
            wire signed [TWIDDLE_W-1:0] tw_re_q;
            wire signed [TWIDDLE_W-1:0] tw_im_q;
            wire signed [DATA_W-1:0] tw_prod_re;
            wire signed [DATA_W-1:0] tw_prod_im;

            twiddle_rom32 #(
                .TWIDDLE_W(TWIDDLE_W),
                .TWIDDLE_FRAC_W(WF_TWIDDLE)
            ) u_twiddle_rom32 (
                .phase(tw_phase),
                .tw_re(tw_re_q),
                .tw_im(tw_im_q)
