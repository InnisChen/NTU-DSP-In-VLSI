%% Step 6: Verify Iterative CORDIC from Verilog Simulation Results
% Reads step6_sim_results.dat written by TESTBED.v (USE_ITERATIVE mode).
% Required result: phase error vs index m (threshold 2^-9 rad).
% Test inputs: m = 0, 3, 6, 9
...
dat_path = fullfile(fileparts(mfilename('fullpath')), ...
           '..', '00_TESTBED', 'src', 'step6_sim_results.dat');

fid = fopen(dat_path, 'r');
if fid < 0
    error('Cannot open %s\nRun Verilog simulation (USE_ITERATIVE) first.', dat_path);
end
...
% Test inputs (must match TESTBED.v, m = 0,3,6,9)
m_idx = [0; 3; 6; 9];
...
fs = 13;
figure('Name','Step6 - Phase Error vs m', 'Position',[100,100,900,450]);
